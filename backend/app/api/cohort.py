...
def scan(db: Session = Depends(get_db)) -> ScanSummary:
    ...
    labs = (
        ...
        .filter(Lab.analyte.in_(REQUIRED_ANALYTES))
        .all()
    )
    if not labs:
        return ScanSummary(total=0, low=0, grey=0, high=0, lost_count=0)
    ...
    for analyte in REQUIRED_ANALYTES:
    ...
    latest = latest.rename(columns={"AST": "ast", "ALT": "alt", "PLT": "plt"})

    scored = score_dataframe(latest)

    reference_date = dt.date.today()
    scored["is_lost"] = scored.apply(
        lambda row: is_lost(row["zone"], row["date"].date(), reference_date), axis=1
    )
